...
// header('Cache-Control: no-store, must-revalidate'); // Replaced by top block
...
if (
  ...
  // Only check auth if accessing a specific doc list
  // This allows unauthenticated access to the editor shell if needed (though typically protected by app)
  // But for Subscript integration, we validate the token.

  // We need to validate the token against the backend?
  // Or just trust the signature if we shared the secret?
  // Currently, clean_auth checks file permissions?
  // Let's rely on the existing auth mechanism if possible, but
  // for now, just let it pass or use the token passed in URL.

  // The previous implementation didn't check the token here in PHP?
  // It relied on backend generating a secure link?
  // The backend generates `index.php?l=...&token=...`.
  // But `index.php` just reads file `l`.
  // And `l` is a path like `../data/email/t4.lst`.
  // If we want to prevent unauthorized access...
  // But assuming internal container network trust + backend auth.
}
...
if (isset($thelist) && $pagenum > count($thelist)) {
...
// $script .= "var brhash = '".$_COOKIE['PHP_AUTH_BR']."';\n"; // Removed cookie reliance?
$script .= "var brhash = '';\n";
$script .= "var list_xmls = [ " . (isset($thelist) ? implode(', ', $thelist) : "") . " ];\n";
$script .= "</script>\n";
...
<body>
  <div id="container">
    <div id="statusBar">
      ...
      <div id="stateInfo">
        <b><span id="modeActive"></span> (<span id="modeElement">-</span>):</b> <span id="selectedType">-</span> <span
          id="selectedId">-</span>
  ...
  <div id="drawer">
    <fieldset id="generalFieldset">
      ...
      <span><b>User:
        ...
        if (isset($_COOKIE['PHP_AUTH_USER']) && $uname === $_COOKIE['PHP_AUTH_USER'])
    ...
    <fieldset id="editModesFieldset">
      <legend class="tooltip-down" data-tooltip="Change via keyboard:&#xa;&#xa0;&#xa0;ctrl[+shift]+, ctrl[+shift]+.">
        Edit modes</legend>
      <div class="radio-set">
        <label id="selMode"><input class="mousetrap" type="radio" name="mode2" value="select" checked="" />
          Select</label>
        ...
        <label id="editAfterCreate"><input class="mousetrap" type="checkbox" checked="" /> Edit mode after
          create</label>
      </div>
      <div class="radio-set">
        ...
        <label id="lineMode"><input class="mousetrap" type="radio" name="mode1" value="line" checked="" />
          TextLine</label>
        ...
        <label id="otherMode" class="tooltip-up"
          data-tooltip="ImageRegion, SeparatorRegion,&#xa;CustomRegion or any custom type">
    ...
    <fieldset id="newPropsFieldset">
      ...
      <div>
        Table size:
        <label id="table-rows"><input class="mousetrap" type="number" name="table-rows" min="1" value="3" />
          rows</label>
        <label id="table-cols"><input class="mousetrap" type="number" name="table-cols" min="1" value="3" />
          columns</label>
      </div>
      <div>
        Group size:
      ...
      <div>
        ...
        <label id="group-init-b"><input class="mousetrap" type="radio" name="group-init" value="box" checked="" />
          box</label>
        <label id="group-init-p"><input class="mousetrap" type="radio" name="group-init" value="polygon" />
          polygon</label>
      </div>
      <div>
        Group member type:
    ...
    <fieldset id="visibilityFieldset">
      ...
      <label id="hide-img"><input class="mousetrap" type="checkbox" checked="" /> Background image pane</label>
      <label id="hide-text-edit"><input class="mousetrap" type="checkbox" checked="" /> Correction verification
        pane</label>
    ...
    <fieldset id="tabCycleFieldset">
      ...
      <div>
        ...
        <label id="tab-sort-text"><input class="mousetrap" type="radio" name="tab-sort-type" value="TextEquiv"
            checked="" /> Text conf</label>
        <label id="tab-sort-coords"><input class="mousetrap" type="radio" name="tab-sort-type" value="Coords" /> Coords
          conf</label>
      </div>
    </fieldset>
  </div>
